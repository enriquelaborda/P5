...
def main():
    ...
    for i in range(num_muestras):
        ...
        fraccion = index - idx_entero
        valor = tabla[idx_entero] * (1.0 - fraccion) + tabla[(idx_entero + 1) % N] * fraccion
        senal_generada[i] = valor
        indices_usados.append((i, index))
        index += step
        if index >= N:
            index -= N
    ...
    ax.plot(x_gen, y_gen, 'rX', markersize=8, label='Señal generada (interpolación lineal)')
    ...
    ax.set_title("Interpolación Lineal para Síntesis por Tabla de Búsqueda")
